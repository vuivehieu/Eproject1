 ...
 $keyword = "";
  if(isset($_GET['keyword'])){
    ...
    $sql = "SELECT * FROM product  WHERE TITLE LIKE '%$keyword%' OR CATEGORY_ID LIKE '%$keyword%' OR MANUFACTURER LIKE '%$keyword%' OR CATEGORY LIKE '%$keyword%' LIMIT $start, $limit";
  	...
  	$sql = "SELECT COUNT(ID) AS total FROM product WHERE CATEGORY LIKE 'Journal'";

		$result = mysqli_query($conn, $sql);
		require_once('pagination.php');


    $sql = "SELECT * FROM product WHERE CATEGORY = 'Journal'
    LIMIT $start, $limit";
  }
...
<!-- <div class="intro-container">
		...
		<div class="intro-container">
			<div class="intro-left" style="background-color: #9dc2bb;">
				<div class="intro-content">
					<div class="intro-title">JOURNALS</div>
					<div class="intro-sub">The potential of journals is unlimited. Whether you want to keep track of the things you've watched, level-up your life planning, prepare for your next adventure, there's a journal to help you on your journey.</div>
				</div>
			</div>
			<div class="intro-right">
				<img src="../Images/Front/Thumbnail/JOUR.jpg">
			</div>
		</div>
