 ...
 $keyword = "";
  if(isset($_GET['keyword'])){
    ...
    $sql = "SELECT * FROM product  WHERE TITLE LIKE '%$keyword%' OR CATEGORY_ID LIKE '%$keyword%' OR MANUFACTURER LIKE '%$keyword%' OR CATEGORY LIKE '%$keyword%' LIMIT $start, $limit";
  	...
  	$sql = "SELECT COUNT(ID) AS total FROM product WHERE CATEGORY LIKE 'Desk Calendar'";

		$result = mysqli_query($conn, $sql);
		require_once('pagination.php');


    $sql = "SELECT * FROM product WHERE CATEGORY = 'Desk Calendar' 
    LIMIT $start, $limit";
  }
...
<!-- <div class="intro-container">
		...
		<div class="intro-container">
			<div class="intro-left" style="background-color: #e36d9a;">
				<div class="intro-content">
					<div class="intro-title">DESK CALENDAR</div>
					<div class="intro-sub">As the name implies, desk calendars are great for desks in the office (whether at work or at home).</div>
				</div>
			</div>
			<div class="intro-right">
				<img src="../Images/Front/Thumbnail/DESK.jpg">
			</div>
		</div>
